...
import { requireVerifiedEmail, tryAuth, authRateLimit } from "../middleware/auth.js";
import {
  badgeCodeSchema,
  normalizeBadgeCode,
...
export const stickerCodesRoutes = new Hono<AppEnv>();

// ── Claim a code (assign to one of the caller's items) ──────────────

stickerCodesRoutes.post(
  "/sticker-codes/:code/claim",
  requireVerifiedEmail,
  authRateLimit,
  async (c) => {
    const code = normalizeBadgeCode(c.req.param("code"));
    ...
    const { itemId } = claimBodySchema.parse(await c.req.json());
    const db = getDb();
    ...
    if (stickerCode.userId !== c.var.userId!) {
    ...
    if (stickerCode.assignedItemId === itemId) {
      return c.json({ success: true, code, itemId, alreadyClaimed: true });
    }
    ...
    // Conditional UPDATE first: it only succeeds while the code is still
    // unassigned, which is what guards against two tabs claiming at once.
    ...
    // Free the item's previous code (if any) and point the item at the new
    // one. Batched so the item never ends up referencing a freed code.
    const pointItemAtCode = db
      .update(items)
    ...
    if (item.badgeCode) {
      await db.batch([
        db
          ...
          .where(
            and(eq(stickerCodes.assignedItemId, itemId), eq(stickerCodes.code, item.badgeCode)),
          ),
        pointItemAtCode,
      ]);
    } else {
      await pointItemAtCode;
    }

    return c.json({ success: true, code, itemId });
  },
);
...
stickerCodesRoutes.get("/sticker-codes/:code/scan", tryAuth, async (c) => {
  const code = normalizeBadgeCode(c.req.param("code"));

  if (!badgeCodeSchema.safeParse(code).success) {
    return c.json({ format: "invalid" as const });
  }

  const db = getDb();
  const currentUserId = c.var.userId ?? null;
  ...
  if (!row) {
    return c.json({ format: "valid" as const, exists: false });
  }

  if (row.voidedAt) {
    return c.json({ format: "valid" as const, exists: true, voided: true });
  }
