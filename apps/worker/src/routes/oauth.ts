...
function createOAuthPendingToken(data: {
  ...
  const signature = createHmac("sha256", config.JWT_PRIVATE_KEY).update(sigData).digest("hex");
...
function verifyOAuthPendingToken(token: string): {
  ...
  const sigData = `${payloadB64}.${expiresAt}.${nonce}.oauth-pending`;
  const expected = createHmac("sha256", config.JWT_PRIVATE_KEY).update(sigData).digest("hex");
...
// POST /auth/google
oauthRoutes.post("/auth/google", authRateLimit, async (c) => {
  const body = oauthCodeSchema.parse(await c.req.json());
  const profile = await exchangeGoogleCode(body.code, body.redirectUri, body.codeVerifier);
  ...
  if (result.needsEmail) {
    return c.json({ needsEmail: true, pendingToken: result.pendingToken });
  }
...
// POST /auth/microsoft
oauthRoutes.post("/auth/microsoft", authRateLimit, async (c) => {
  const body = oauthCodeSchema.parse(await c.req.json());
  const profile = await exchangeMicrosoftCode(body.code, body.redirectUri, body.codeVerifier);
...
oauthRoutes.post("/auth/facebook", authRateLimit, async (c) => {
  const body = oauthCodeNoPkceSchema.parse(await c.req.json());
  const profile = await exchangeFacebookCode(body.code, body.redirectUri);
...
oauthRoutes.post("/auth/oauth-complete", authRateLimit, async (c) => {
  ...
  c.executionCtx.waitUntil(
    sendEmail(
      ...
      console.error("Failed to send verification email after OAuth complete", err);
    }),
  );
